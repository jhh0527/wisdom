#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GUI 진입점 — ``dist/7_4_mp4Merge_gui.exe`` 우선."""
...
def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "7_4_mp4Merge_gui.exe"
...
def main() -> None:
    ...
    if getattr(sys, "frozen", False):
        try:
            from mp4_merge.gui_app import main as gui_main
    ...
    exe = _dist_gui_exe()
    use_source = os.environ.get("MP4_MERGE_GUI_SOURCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _show_error_dialog() -> None:
    try:
        ...
        r = tk.Tk()
        r.withdraw()
        messagebox.showerror("7_4 mp4Merge", traceback.format_exc())
        r.destroy()
    except Exception:
        pass
