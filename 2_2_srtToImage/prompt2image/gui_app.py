...
def main(*, container: tk.Misc | None = None) -> None:
    ...
    apply_window_chrome(
        root,
        standalone,
        title=f"2_2_srtToImage GUI {__version__}",
        minsize=(720, 520),
        geometry="840x600",
    )
    ...
    out_dir = tk.StringVar(value=str(resolve_module_output("2_2_srtToImage")))
    ...
    def pick_out() -> None:
        init = folder_dialog_initial(
            Path(out_dir.get().strip()) if out_dir.get().strip() else resolve_module_output("2_2_srtToImage"),
        )
