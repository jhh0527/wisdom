...
from shortvid.assets import ensure_layout, make_placeholder_png, make_silent_mp3
from shortvid.compose_render import (
    default_compose_audio,
    default_compose_srt,
    render_compose_from_assets,
)
from shortvid.ffmpeg_render import render_project
from shortvid.schema import DEFAULT_OUTRO_TEXT, RenderSettings, load_project, save_project
from shortvid.script_parse import script_md_to_doc
...
from shortvid.repo_paths import (
    ...
    default_shortvid_compose_mp4_name,
)
...
def cmd_init(project: Path) -> int:
    ...
    print(f"예시 스크립트: {sp}")
    print(
        "다음(한 줄 예): python -m shortvid all -p .  "
...
def cmd_compose(
    ...
    *,
    ...
    if not img_dir.is_dir():
        ...
        return 1
    outp = (out.resolve() if out else default_shortvid_compose_mp4())
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(
        ...
        description=(
            ...
            "산출물만 합칠 때는 compose (MP3+SRT+images/srt_NN); 기본 MP4 출력은 wisdom/4_2_ShortVideo/output/shorts_yyyymmdd.mp4. 자막만 갱신: subtitles."
        ),
    ...
    cp.add_argument(
        ...
        type=Path,
        default=None,
        help=f"출력 MP4 (기본: wisdom/4_2_ShortVideo/output/{default_shortvid_compose_mp4_name()})",
    )
    cp.add_argument(
        "--outro-text",
        type=str,
