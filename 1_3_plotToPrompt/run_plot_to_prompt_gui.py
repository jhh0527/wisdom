#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""1_3_plotToPrompt GUI 진입점."""

from __future__ import annotations

import os
import subprocess
import sys
import traceback
from pathlib import Path


def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "1_3_plotToPrompt_gui.exe"


def main() -> None:
    _WISDOM = Path(__file__).resolve().parent.parent
    if str(_WISDOM) not in sys.path:
        sys.path.insert(0, str(_WISDOM))
    from wisdom_bootstrap import run as wisdom_run

    work = wisdom_run(__file__)

    if getattr(sys, "frozen", False):
        try:
            from plot_to_prompt.gui_app import main as gui_main

            gui_main()
        except Exception:
            try:
                import tkinter as tk
                from tkinter import messagebox

                r = tk.Tk()
                r.withdraw()
                messagebox.showerror("1_3_plotToPrompt GUI", traceback.format_exc())
                r.destroy()
            except Exception:
                pass
            raise
        return

    root = Path(__file__).resolve().parent
    exe = _dist_gui_exe()
    use_source = os.environ.get("PLOT_TO_PROMPT_GUI_SOURCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
    )
    if not use_source and exe.is_file():
        r = subprocess.run([str(exe)], cwd=str(work))
        raise SystemExit(r.returncode or 0)

    try:
        sys.path.insert(0, str(root))
        from plot_to_prompt.gui_app import main as gui_main

        gui_main()
    except Exception:
        traceback.print_exc()
        raise


if __name__ == "__main__":
    main()
