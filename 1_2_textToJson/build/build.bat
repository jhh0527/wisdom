...
echo [1_2_textToJson] PyInstaller 빌드 시작...
set "PROOT=%~dp0.."
...
set "DISTEXE=!PROOT!\dist\1_2_textToJson_gui.exe"
taskkill /F /IM 1_2_textToJson_gui.exe >nul 2>&1
ping -n 2 127.0.0.1 >nul
if exist "!DISTEXE!" del /F /Q "!DISTEXE!" >nul 2>&1
if exist "%~dp0work" rmdir /s /q "%~dp0work"

"!PYEXE!" -m PyInstaller --clean --noconfirm --distpath "!PROOT!\dist" --workpath "%~dp0work" "%~dp0text_to_json_gui.spec"
if errorlevel 1 exit /b 1

if not exist "!DISTEXE!" (
  echo dist\1_2_textToJson_gui.exe 를 찾을 수 없습니다.
  exit /b 1
)
