...
echo [5_video] PyInstaller 빌드 시작...
...
echo   ※ 5_video_gui.exe 가 실행 중이면 ^(접근 거부^) 빌드가 실패합니다. GUI 창을 닫은 뒤 build.bat 을 다시 실행하세요.
if exist "!PROOT!\dist\5_video_gui.exe" (
  del /f /q "!PROOT!\dist\5_video_gui.exe" 2>nul
  if exist "!PROOT!\dist\5_video_gui.exe" (
    echo [경고] 기존 5_video_gui.exe 를 지울 수 없습니다. 프로세스를 종료한 뒤 build.bat 을 다시 실행하세요.
    exit /b 1
  )
)
