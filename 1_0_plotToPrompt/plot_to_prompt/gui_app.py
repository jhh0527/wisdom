# -*- coding: utf-8 -*-
"""1_0_plotToPrompt GUI — 줄거리→BRIEF · BRIEF→합본·tts."""
...
def main(*, container: tk.Misc | None = None) -> None:
    ...
    apply_window_chrome(
        root,
        standalone,
        title=f"1_0_plotToPrompt {__version__}",
        minsize=(720, 620),
        geometry="880x720",
    )
    ...
    chapter_var = tk.StringVar(value=cfg.get("last_chapter") or "23")
    meta_var = tk.StringVar(value="")
    status_var = tk.StringVar(value="장 번호 + 줄거리(~700자). 로그는 하단.")
    ...
    def pick_work() -> None:
        ...
        p = filedialog.askdirectory(title="작업 루트 (tts 상위)", initialdir=init)
        if p:
            touch_workspace_from_path(p)
            work_var.set(p)
            diag_log.log(f"작업 루트={p}")
    ...
    ttk.Label(r2, text="→ tts/{장}.txt").pack(side=tk.LEFT, padx=(8, 0))

    r3 = ttk.Frame(path_fr)
    r3.pack(fill=tk.X, pady=2)
    ttk.Label(r3, text="장 번호", width=10).pack(side=tk.LEFT)
    ttk.Entry(r3, textvariable=chapter_var, width=8).pack(side=tk.LEFT, padx=4)
    ttk.Label(r3, textvariable=meta_var, wraplength=620).pack(side=tk.LEFT, padx=(12, 0))

    def _persist() -> None:
        save_gui_settings(
            novel_root=novel_var.get().strip(),
            last_chapter=chapter_var.get().strip(),
            work_root=work_var.get().strip(),
        )

    def _chapter_int() -> int:
        n = int(chapter_var.get().strip())
        if n < 1:
            raise ValueError("장 번호는 1 이상")
        return n
    ...
    def refresh_meta(*_a: object) -> None:
        ...
        meta_var.set("자동: " + line)
        diag_log.log(f"메타 ch={ch} {line}")

    chapter_var.trace_add("write", refresh_meta)
    ...
    def _build_from_plot() -> str | None:
        try:
            ...
            messagebox.showwarning("장 번호", "장 번호는 정수여야 합니다.")
            diag_log.log("BRIEF 중단: 장 번호 오류")
            return None
    ...
    def do_brief() -> None:
        try:
            ...
            if out is None:
                status_var.set("BRIEF 생성됨 · 저장 실패(장 번호 확인)")
                _idle("저장 실패")
                return
            _set_progress(4, 4, "저장 완료")
            status_var.set(f"BRIEF 생성·자동 저장: {out}")
            _idle("BRIEF 완료")
    ...
    def do_build_packet() -> None:
        try:
            ...
            messagebox.showerror("장 번호", str(e))
            return
        novel = novel_var.get().strip()
        work = work_var.get().strip()
        if not novel or not Path(novel).is_dir():
            ...
            return
        if not work:
            messagebox.showerror("작업 루트", "작업 루트(tts 상위)를 선택하세요.")
            return
        _busy("합본 조립…")
        try:
