...
def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "1_0_plotToPrompt_gui.exe"
...
def main() -> None:
    ...
    if getattr(sys, "frozen", False):
        try:
            from plot_to_prompt.gui_app import main as gui_main
            ...
            try:
                ...
                r = tk.Tk()
                r.withdraw()
                messagebox.showerror("1_0_plotToPrompt GUI", traceback.format_exc())
                r.destroy()
            except Exception:
                pass
            raise
    ...
    exe = _dist_gui_exe()
    use_source = os.environ.get("PLOT_TO_PROMPT_GUI_SOURCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
