# -*- coding: utf-8 -*-
"""1_5_textToJson GUI — 루트/tts txt + 루트/md 지침 → Genspark → tts/*.json."""
...
def main(*, container: tk.Misc | None = None) -> None:
    ...
    apply_window_chrome(
        root,
        standalone,
        title=f"1_5 textToJson {__version__}",
        minsize=(780, 580),
        geometry="920x760",
    )
    ...
    def export_json(source: str) -> Path | None:
        ...
        try:
            ...
            safe_messagebox(root, "showwarning", "1_5 textToJson", str(e))
            return None
    ...
    def _prepare_convert() -> tuple[str, str, Path, Path, str] | None:
        ...
        if not has_playwright():
            safe_messagebox(
                root, "showerror", "1_5 textToJson", "Playwright가 필요합니다."
            )
            return None
        ...
        def work() -> None:
            try:
                ...
                def fail() -> None:
                    set_busy(False)
                    diag_log.log(f"오류: {e}")
                    safe_messagebox(root, "showerror", "1_5 textToJson", str(e))
    ...
    btn_browser = ttk.Button(act, text="실행", command=_start_convert)
