...
def module_root() -> Path:
    """워크스페이스의 ``1_5_textToJson`` (없으면 패키지/번들)."""
    try:
        from wisdom_root import module_dir

        src = module_dir("1_5_textToJson")
        if src.is_dir():
            return src.resolve()
    except Exception:
        pass
...
def module_md_dir() -> Path:
    """모듈 ``1_5_textToJson/md`` — 소스 트리 우선 (exe ``_MEI`` 임시폴더보다 앞)."""
    src = module_root() / "md"
    if src.is_dir():
        return src.resolve()
    if getattr(sys, "frozen", False):
        meipass = getattr(sys, "_MEIPASS", "")
        if meipass:
            for sub in ("md_1_5_textToJson", "md_1_2_textToJson", "md"):
                p = Path(meipass) / sub
                if p.is_dir():
                    return p
...
def default_dialogue_json_md() -> Path | None:
    """변환 지침 디폴트: ``…/1_5_textToJson/md/dialogue_json.txt``."""
...
def is_canonical_dialogue_md(path: Path | str | None) -> bool:
    ...
    try:
        ...
        if guide is not None and p.resolve() == guide.resolve():
            return True
        return (
            ("1_5_textToJson" in s or "1_2_textToJson" in s)
            and "/md/" in s.casefold()
        )
    except OSError:
        return False
