...
echo PyInstaller 실행 ^(GUI: "!PROOT!\dist\3_2_pngToJpg_gui.exe"^)...
echo   ※ 3_2_pngToJpg_gui.exe 가 실행 중이면 빌드가 실패할 수 있습니다. GUI 창을 닫은 뒤 다시 실행하세요.
if exist "!PROOT!\dist\3_2_pngToJpg_gui.exe" (
  del /f /q "!PROOT!\dist\3_2_pngToJpg_gui.exe" 2>nul
  if exist "!PROOT!\dist\3_2_pngToJpg_gui.exe" (
    echo [경고] 기존 3_2_pngToJpg_gui.exe 를 지울 수 없습니다. 프로세스를 종료한 뒤 build.bat 을 다시 실행하세요.
    exit /b 1
  )
)
...
echo.
echo 완료:
echo   GUI  "!PROOT!\dist\3_2_pngToJpg_gui.exe"
