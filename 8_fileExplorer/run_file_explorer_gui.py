...
- ``dist/8_fileExplorer_gui.exe`` 가 있으면 우선 실행 (빌드 산출물).
- 소스 강제: 환경 변수 ``FILE_EXPLORER_GUI_SOURCE=1``
"""
...
def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "8_fileExplorer_gui.exe"
...
def main() -> None:
    ...
    if getattr(sys, "frozen", False):
        try:
            from file_explorer.gui_app import main as gui_main
    ...
    exe = _dist_gui_exe()
    use_source = os.environ.get("FILE_EXPLORER_GUI_SOURCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _show_error_dialog() -> None:
    try:
        ...
        r = tk.Tk()
        r.withdraw()
        messagebox.showerror("8_fileExplorer", traceback.format_exc())
        r.destroy()
    except Exception:
        pass
