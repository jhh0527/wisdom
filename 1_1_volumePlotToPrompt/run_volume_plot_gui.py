#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""1_1_volumePlotToPrompt GUI 진입점."""
...
def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "1_1_volumePlotToPrompt_gui.exe"
...
def main() -> None:
    ...
    if getattr(sys, "frozen", False):
        try:
            from volume_plot.gui_app import main as gui_main
            ...
            try:
                ...
                r = tk.Tk()
                r.withdraw()
                messagebox.showerror("1_1_volumePlotToPrompt GUI", traceback.format_exc())
                r.destroy()
            except Exception:
                pass
            raise
    ...
    exe = _dist_gui_exe()
    use_source = os.environ.get("VOLUME_PLOT_GUI_SOURCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
