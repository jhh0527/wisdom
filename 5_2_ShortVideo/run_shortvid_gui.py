#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""5_2_ShortVideo GUI 진입점 — ``dist/5_2_shortvideo_gui.exe`` 우선."""
...
def _dist_gui_exe() -> Path:
    return Path(__file__).resolve().parent / "dist" / "5_2_shortvideo_gui.exe"
    ...
    if getattr(sys, "frozen", False):
        try:
            ...
            try:
                ...
                r = tk.Tk()
                r.withdraw()
                messagebox.showerror("5_2_ShortVideo", traceback.format_exc())
                r.destroy()
            except Exception:
                pass
            raise
